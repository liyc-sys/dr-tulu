...
class InterleavedTextFixer:
    def __init__(self, openrouter_api_key: str, max_workers: int = 20):
...
Please provide the completion:"""
        ...
        try:
            ...
            # Return simple closing tag and failure status
            return "</answer>", False
    ...
    def fix_single_interleaved_text(self, question: str, interleaved_text: str) -> Tuple[str, bool]:
        ...
        print("Detected truncated interleaved_text, analyzing...")
        ...
        if self.needs_completion(incomplete_section):
            ...
            if api_success:
                print("API completion completed")
            else:
                print("API completion failed - using fallback")
                
            return fixed_text, api_success
        else:
            print("Content appears complete, just adding closing tag...")
            # Just add the missing closing tag
            fixed_text = interleaved_text + "</answer>"
            print("Closing tag added")
    ...
    def archive_progress(self):
        ...
        if os.path.exists(self.progress_file):
            ...
            archive_filename = f"fix_progress_backup_{timestamp}.json"
            
            try:
                os.rename(self.progress_file, archive_filename)
                print(f"进度文件已备份为: {archive_filename}")
            except Exception as e:
                print(f"备份进度文件时出错: {e}")
    ...
    def process_single_line(self, line_data: Dict) -> Tuple[Dict, bool]:
        ...
        try:
            ...
            print(f"Error in process_single_line: {e}")
        ...
        with ThreadPoolExecutor(max_workers=self.max_workers) as executor:
            ...
            for future in as_completed(future_to_index):
                ...
                try:
                    ...
                    print(f"Error processing future for line {start_line + index}: {e}")
    ...
    def process_jsonl_file(self, input_file: str, output_file: str):
                ...
                for failed_line_num in retry_lines:
                    if failed_line_num <= len(lines):
                        try:
                            ...
                            print(f"Error loading retry line {failed_line_num}: {e}")
            
            if retry_data:
                print(f"并行重试 {len(retry_data)} 个失败条目...")
        ...
        print(f"继续处理新行，从第 {start_line} 行开始 (并行处理)...")
        ...
        with open(input_file, 'r', encoding='utf-8') as infile:
            ...
            for batch_start in range(start_line - 1, total_lines, batch_size):
                ...
                print(f"处理批次: 行 {batch_start + 1} 到 {batch_end} ({len(batch_lines)} 行)")
                ...
                print(f"批次完成: 成功 {batch_success}, 失败 {batch_failure}")
                ...
                if current_line % save_interval == 0:
                    ...
                    print(f"进度已保存，处理到第 {current_line} 行")
        ...
        for line_num, line in enumerate(infile, 1):
            try:
                ...
                if 'trajectory' in data and 'interleaved_text' in data['trajectory']:
                    ...
                    temp_fixer = InterleavedTextFixer("dummy_key")
...
def main():
    ...
    if len(sys.argv) > 1:
        if sys.argv[1] == "--preview":
            ...
            # Create a dummy fixer to clear progress
            dummy_fixer = InterleavedTextFixer("dummy_key")
            dummy_fixer.clear_progress()
            ...
            import glob
            backup_files = glob.glob("fix_progress_backup_*.json")
            if backup_files:
                print("Found backup files:")
                for backup in sorted(backup_files):
                    # Extract timestamp from filename
                    try:
                        timestamp_str = backup.replace("fix_progress_backup_", "").replace(".json", "")
            ...
            print("  python fix_interleaved_text.py --clear-progress   # Clear saved progress")
            print("  python fix_interleaved_text.py --list-backups     # List all backup files")
            print("  python fix_interleaved_text.py --clean-backups    # Delete all backup files")
            print("  python fix_interleaved_text.py --help             # Show this help")
            print("")
            print("Backup Management:")
            print("  - Completed progress files are automatically backed up with timestamp")
            print("  - Failed progress files are kept for resuming")
            print("  - Use --list-backups to see all backup files")
            print("  - Use --clean-backups to remove old backup files")
            print("")
            print("Examples:")
            print("  python fix_interleaved_text.py --workers=30       # Use 30 concurrent threads")
            print("  python fix_interleaved_text.py --list-backups     # Show backup files")
            return
    ...
    fixer = InterleavedTextFixer(api_key, max_workers=workers)
