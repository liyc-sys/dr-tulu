#!/bin/bash
# DR-Tulu 训练脚本 - 使用 OpenRouter 直接调用（不使用 LiteLLM）
# 这个脚本是 train_dr_tulu.sh 的修改版，使用更稳定的 OpenRouter 直接调用
...
# OpenAI API 配置（用于 OpenRouter）
export OPENAI_API_KEY="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"
export OPENAI_API_BASE="https://openrouter.ai/api/v1"

# ========== 新增：使用 OpenRouter 直接调用 ==========
export USE_OPENROUTER_DIRECT=true

# 使用完整的模型名（包含 provider 前缀）
# 推荐使用 gpt-4o-mini：便宜、快速、效果好
export RUBRIC_JUDGE_MODEL=openai/gpt-5-mini
export RUBRIC_GENERATION_MODEL=openai/gpt-5-mini
...
exp_name="dr-tulu-openrouter"
log_file="train_${exp_name}_$(date +%Y%m%d_%H%M%S).log"

echo "日志文件将保存到: ${log_file}"
echo "========== OpenRouter 直接调用配置 =========="
echo "USE_OPENROUTER_DIRECT: $USE_OPENROUTER_DIRECT"
