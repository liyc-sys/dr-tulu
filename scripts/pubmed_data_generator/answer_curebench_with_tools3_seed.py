...
读取 curebench JSONL，调用本地模型生成带工具调用的答案
"""
import asyncio
import json
import os
...
class CureBenchAnswerer:
    ...
    def __init__(
        ...
        try:
            ...
            print("  ⚠ tiktoken 未安装，将使用字符数估算 token（约 3 字符 = 1 token）")
    ...
    async def answer_question(self, question_data: Dict) -> CureBenchResult:
        ...
        while True:
            ...
            if current_tokens > self.max_tokens - 3000:
                ...
                messages.append({
                    "role": "user",
                    "content": f"⚠️ CRITICAL: Context length ({current_tokens}/{self.max_tokens} tokens) is near the limit. You MUST stop all tool calls and provide your final answer IMMEDIATELY using the <answer> tag with <choice>A/B/C/D</choice>. Summarize what you have found and give your best answer."
                })
            ...
            if tool_call_matches and not token_limit_reached:
                ...
                if "<answer>" in content:
                    ...
                    # 已达到 token 限制，但模型没有给出答案，尝试从内容中提取
                    interleaved_parts.append(content)
                    print(f"  ⚠️ Token 限制已达到，但模型未给出答案，尝试提取")
                    # 尝试找到任何 A/B/C/D 的提及
                    letter_match = re.search(r'\b([A-D])\b', content)
                    if letter_match:
...
class CureBenchRunner:
    ...
    def __init__(
        ...
        self.output_file = os.path.join(
            self.output_dir,
            f"curebench_results_{timestamp}_{file_suffix}.jsonl"
        )
        self.stats_file = os.path.join(
            self.output_dir,
            f"curebench_stats_{timestamp}_{file_suffix}.json"
        )
    ...
    async def run_evaluation(self, concurrency: int = 5, limit: int = None):
        """运行评测"""
        print("\n" + "=" * 60)
        print("CureBench 评测 - 使用本地模型 + MCP 工具")
        print("=" * 60)
