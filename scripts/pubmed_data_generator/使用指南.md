# DR-Tulu-8B 本地模型轨迹生成 - 使用指南

## 功能说明

从已有的questions JSONL文件读取问题，使用本地部署的DR-Tulu-8B模型生成轨迹，**不生成rubrics**。
...
for port in 8000 8001 8002 8009 8004 8005 8006 8007; do
    curl -s http://localhost:$port/v1/models >/dev/null && echo "✓ $port" || echo "✗ $port"
done
```
...
uv run --project /workspace/math_science_data/lyc/1205/dr-tulu/rl/open-instruct bash run_multi_instance.sh \
    /workspace/math_science_data/lyc/1205/dr-tulu/交付数据/pubmed_trajectory_20251218_060613_no_rubrics_incremental.jsonl
```
...
| 参数 | 说明 | 默认值 |
|------|------|--------|
| `--questions-file` | questions JSONL文件路径（必填） | - |
| `--local-model-url` | 本地模型API地址 | `http://localhost:8000/v1` |
| `--model-name` | 模型名称 | `DR-Tulu-8B` |
| `--concurrency` | 并发数 | `5` |
| `--limit` | 限制处理数量（测试用） | `None`（全部） |
...
在 `/workspace/math_science_data/lyc/1205/dr-tulu/pubmed_training_data/` 目录：

- **多实例模式**：`pubmed_trajectory_YYYYMMDD_HHMMSS_DR-Tulu-8B_multi_instance.jsonl`
- **单实例模式**：`pubmed_trajectory_YYYYMMDD_HHMMSS_DR-Tulu-8B_incremental.jsonl`
- **统计文件**：`trajectory_stats_YYYYMMDD_HHMMSS_DR-Tulu-8B.json`
...
# 检验是否超长
python /workspace/math_science_data/lyc/1205/dr-tulu/交付数据/check_trajectory_token.py
```
...
```bash
for port in 8000 8001 8002 8009 8004 8005 8006 8007; do
    curl -s http://localhost:$port/v1/models >/dev/null && echo "✓ Port $port OK" || echo "✗ Port $port FAIL"
done
```
...
**Q: 如何调整速度？**
- 修改 `run_multi_instance.sh` 第14行的 `CONCURRENCY_PER_INSTANCE`
- 默认3，可改为5-8（如果GPU内存足够）
