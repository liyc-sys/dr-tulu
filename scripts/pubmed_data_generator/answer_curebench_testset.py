...
基于 answer_curebench_with_tools.py 修改，支持无 correct_answer 的 testset
"""
import asyncio
import json
import glob
...
class CureBenchRunner:
    ...
    def __init__(
        ...
        prompt_version: str = "original"  # 标识使用哪个 prompt 版本
    ):
    ...
    async def run_evaluation(self, concurrency: int = 5, limit: int = None):
        print("\n" + "=" * 60)
        print(f"CureBench Testset 评测 - Prompt版本: {self.prompt_version}")
        print("=" * 60)
        ...
        # 检查是否有 correct_answer（判断是 valset 还是 testset）
    ...
    def save_stats(self, has_correct_answer: bool = True):
        ...
        if has_correct_answer:
            for qtype in by_type:
                ...
                del by_type[qtype]["answers"]  # 有正确答案时不需要保存所有答案
...
async def main():
    ...
    parser = argparse.ArgumentParser(description="使用本地模型 + MCP 工具回答 CureBench Testset 问题")
