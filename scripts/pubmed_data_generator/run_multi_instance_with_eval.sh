...
# OpenRouter API密钥（使用OPENROUTER_API_KEY环境变量）
# 需要在运行前设置: export OPENROUTER_API_KEY="sk-or-v1-..."
...
# 合并结果
echo "合并生成的轨迹评估文件..."
MERGED_FILE="$OUTPUT_DIR/pubmed_trajectory_evaluated_$(date +%Y%m%d_%H%M%S)_${MODEL_NAME}_multi_instance.jsonl"

# 找到所有生成的轨迹评估文件并合并
find "$OUTPUT_DIR" -name "pubmed_trajectory_evaluated_*_${MODEL_NAME}_port*_incremental.jsonl" -type f | while read file; do
