...
基于 answer_curebench_testset3.py（带更详细指南和约束的 prompt + 重复检测）
LLM 调用改为 OpenRouter API
"""
import asyncio
import json
import os
...
# 来自 answer_curebench_testset3.py 的 SYSTEM_PROMPT，增加重复检测提示
...
1. **fda_drug_search (PRIMARY TOOL)**:
   - **USE FIRST** for any question involving **specific medications, drugs, pharmaceuticals**
   ...
   - **NOTE**: For general regulatory standards (e.g., sunscreen SPF guidelines) rather than specific drug prescribing information, use google_search instead
...
3. **google_search (LAST RESORT)**:
   ...
   - **General regulatory standards** (e.g., sunscreen SPF guidelines, medical device classifications) that are not about a specific drug's prescribing information
...
6. **Avoid redundancy**: Check previous tool calls to ensure you do not issue duplicate requests
7. **Only escalate if needed**: If FDA doesn't provide enough info, then use browse_webpage or google_search
8. **Provide final answer**: Use the <answer> format with clear reasoning
9. **CRITICAL**: If you receive a "DUPLICATE QUERY DETECTED" warning, you MUST try a different approach immediately
...
- ✅ **CRITICAL**: Query parameter must contain ONLY the exact drug name (e.g., 'Gadavist'). Never include symptoms, conditions, or natural language questions in the query field.
- ✅ Use the focus parameter to target specific label sections (adverse reactions, indications, etc.)
...
Tool Use Constraint: When using fda_drug_search, the query argument must strictly contain ONLY the drug's brand name or generic name. Do not include keywords like "warning", "label", "injection", or natural language phrases in the query field. Use the focus parameter to target specific sections.
...
# 导入 OpenRouter 基类
from answer_curebench_testset_openrouter import CureBenchResult, CureBenchAnswerer as BaseAnswerer, CureBenchRunner as BaseRunner


class CureBenchAnswerer(BaseAnswerer):
    """使用 prompt3 的 OpenRouter 回答器，带重复检测"""
    ...
    def _make_query_key(self, tool_name: str, query: str, parameters: Dict[str, Any]) -> str:
        ...
        if "focus" in parameters:
            key_parts.append(f"focus={parameters['focus']}")
        return "|".join(key_parts)

    def _generate_duplicate_warning(self, tool_name: str, query: str, parameters: Dict[str, Any]) -> str:
        """生成重复查询警告消息"""
        focus_str = f" with focus='{parameters.get('focus', '')}'" if 'focus' in parameters else ""
        return f"""<tool_output>
⚠️ DUPLICATE QUERY DETECTED ⚠️

You have already called the tool '{tool_name}' with the same query: "{query}"{focus_str}
...
2. If the previous result was insufficient, try a DIFFERENT approach:
   - Use a different tool (e.g., google_search instead of fda_drug_search)
   - Use a different query or drug name
   - Change the 'focus' parameter to target a different section
...
async def answer_question_with_prompt3(self, question_data: Dict) -> CureBenchResult:
...
CureBenchAnswerer.answer_question = answer_question_with_prompt3


class CureBenchRunner(BaseRunner):
    """使用 prompt3 的 OpenRouter 运行器"""
    def __init__(self, *args, **kwargs):
        kwargs['prompt_version'] = 'prompt3'
        super().__init__(*args, **kwargs)
        self.answerer = CureBenchAnswerer(
            api_key=self.answerer.api_key,
...
async def main():
    ...
    parser = argparse.ArgumentParser(description="使用 OpenRouter API + MCP 工具回答 CureBench Testset 问题 (Prompt3 + 重复检测)")
    ...
    parser.add_argument("--model-name", type=str, default="seed1.8")
    ...
    print(f"Prompt版本: prompt3 (带重复检测)")
